...
const CURLE_TOO_LARGE = 100 % UInt32
const CURLE_ECH_REQUIRED = 101 % UInt32
const CURL_LAST = 102 % UInt32
...
const CURLOPT_SERVER_RESPONSE_TIMEOUT_MS = 324 % UInt32
const CURLOPT_ECH = 10325 % UInt32
const CURLOPT_TCP_KEEPCNT = 326 % UInt32
const CURLOPT_LASTENTRY = 327 % UInt32
...
const CURLINFO_QUEUE_TIME_T = 6291521 % UInt32
const CURLINFO_USED_PROXY = 2097218 % UInt32
const CURLINFO_LASTONE = 66 % UInt32
...
const CURLVERSION_TWELFTH = 11 % UInt32
const CURLVERSION_LAST = 12 % UInt32
...
function curl_multi_get_handles(multi_handle)
    @ccall libcurl.curl_multi_get_handles(multi_handle::Ptr{CURLM})::Ptr{Ptr{CURL}}
end
...
function curl_multi_waitfds(multi, ufds, size, fd_count)
    @ccall libcurl.curl_multi_waitfds(multi::Ptr{CURLM}, ufds::Ptr{curl_waitfd}, size::Cuint, fd_count::Ptr{Cuint})::CURLMcode
end
...
const CURLUE_TOO_LARGE = 31 % UInt32
const CURLUE_LAST = 32 % UInt32
...
# automatic type deduction for variadic arguments may not be what you want, please use with caution
@generated function curl_mprintf(format, va_list...)
        :(@ccall(libcurl.curl_mprintf(format::Ptr{Cchar}; $(to_c_type_pairs(va_list)...))::Cint))
    end

# automatic type deduction for variadic arguments may not be what you want, please use with caution
@generated function curl_mfprintf(fd, format, va_list...)
        :(@ccall(libcurl.curl_mfprintf(fd::Ptr{Libc.FILE}, format::Ptr{Cchar}; $(to_c_type_pairs(va_list)...))::Cint))
    end

# automatic type deduction for variadic arguments may not be what you want, please use with caution
@generated function curl_msprintf(buffer, format, va_list...)
        :(@ccall(libcurl.curl_msprintf(buffer::Ptr{Cchar}, format::Ptr{Cchar}; $(to_c_type_pairs(va_list)...))::Cint))
    end

# automatic type deduction for variadic arguments may not be what you want, please use with caution
@generated function curl_msnprintf(buffer, maxlength, format, va_list...)
        :(@ccall(libcurl.curl_msnprintf(buffer::Ptr{Cchar}, maxlength::Csize_t, format::Ptr{Cchar}; $(to_c_type_pairs(va_list)...))::Cint))
    end

# automatic type deduction for variadic arguments may not be what you want, please use with caution
@generated function curl_maprintf(format, va_list...)
        :(@ccall(libcurl.curl_maprintf(format::Ptr{Cchar}; $(to_c_type_pairs(va_list)...))::Ptr{Cchar}))
    end

const LIBCURL_COPYRIGHT = "Daniel Stenberg, <daniel@haxx.se>."

const LIBCURL_VERSION = "8.9.1"

const LIBCURL_VERSION_MAJOR = 8

const LIBCURL_VERSION_MINOR = 9

const LIBCURL_VERSION_PATCH = 1

const LIBCURL_VERSION_NUM = 0x00080901

const LIBCURL_TIMESTAMP = "2024-07-31"
...
const CURL_SSLVERSION_MAX_LAST = CURL_SSLVERSION_LAST << 16
...
const CURLVERSION_NOW = CURLVERSION_TWELFTH
...
# Skipping MacroDefinition: CURL_TEMP_PRINTF ( fmt , arg ) __attribute__ ( ( format ( printf , fmt , arg ) ) )
